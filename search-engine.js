...
// Default Candidate Resume Context for the AI ATS to evaluate against
...
// Use resume context from environment variable if configured, otherwise fall back to default
const getResumeContext = () => {
  return process.env.RESUME_CONTEXT || DEFAULT_RESUME_CONTEXT;
};
...
export async function fetchJobsFromJSearch(config) {
  ...
  const resumeSnippet = resume.trim().substring(0, 80).replace(/\n/g, ' ');
  console.log(`📄 Resume Context Loaded: "${resumeSnippet}..."`);
